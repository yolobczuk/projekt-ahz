...
time <- diabetic %>% dplyr::select(c(time))
...
Pierwsza dwa histogramy przedstawiają rozkład wartośći w zmiennych **age** oraz **time**. Zauważyć możemy, iż duża liczba pacjentów ze zdiagnozowaną chorbą znajduje się pomiędzy 8 a 20 rokiem życia.  wiek około 45 lat skupio wokół siebie sporo pacjentów z grupy osób dorosłych. Najmniej zdiagnozowano osób w wieku powyżej 55 lat. Obok widoczny jest wykres częstotliowści liczby miesięcy do momentu utraty wzroku. Najczęstszymi wartościami czasu do utraty wzroku są 3 do 4 oraz 38 do 42 miesięcy od diagnozy. Aczkolwiek, rozkład zmiennej time jest bardzo nieregularny i ciężko zauważyć tutaj wyraźne okresy koncentracji. 
...
Nie rozrysowujemy wykresów dla zmiennych które maja większa ilość kategorii, gdyż wykresy w tym wypadku sa skrajnie nieczytelne.
