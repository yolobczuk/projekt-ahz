...
author: "Marcel Gęsicki, Jan Jarosz, Paweł Kawecki, Wojciech Sobczuk (261172)"
output:
  html_document:
    df_print: paged
    code_folding: hide
...
Analizę rozpoczniemy od zagłębienia się w dane i dokonania transformacji, jeśli takowe będą potrzebne.
...
Powyżej przedstawione zostały wykresy prawdopodobieństwa zachowania wzroku z wykorzystaniem estymatora Kaplana-Meiera oraz Nelsona-Aalena. Widzimy w przypadku krzywej zachowania wzroku dla zmiennej **trt**, że prawdopodobeństwo zachowania wzroku stopniowo maleje. Wartości obu estymatorów są bardzo podobno co potwierdza fakt, że obie linie się nakładają w obrębie grup. Zauważyć można jednak duży rozstrzał w prawdopodobieństwach zachowania wzroku dla grup z terapią oraz bez niej. Zostanie to głębiej zbadane w dalszej części pracy. 
...
Na wszystkich trzech powyższych wykresach również nie widać różnic między estymatorami dla grup. Dlatego też w dalszej części pracy używać będziemy estymatorów Kaplana-Meiera.
...
Z powyższych testów wynika, że dla wszystkich wariantów transformacji czasów przetrwania, funkcja skumulowanego hazardu jest proporcjonalna dla zmiennych w czasie. Wykresy skalowanych reszt Schoenfelda potwierdzają ten fakt. Przystąpimy teraz do wyrzucenia ze zbioru wartości odstających, wyliczonych na podstawie reszt z wyestymowanego wcześniej modelu. Posłużymy się tutaj odchyleniami standardowymi od wartości pradziwych oraz statystyką dfbeta, która służy do wykrywania wpływowych obserwacji.
