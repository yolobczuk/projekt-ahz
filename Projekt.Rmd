...
Badany w niniejszym opracowaniu zbiór danych składa się z danych medycznych odnoszących się do wyników koagulacji laserowej w leczeniu ratinopatii cukrzycowej. Samo schorzenie spowodowane jest uszkodzeniem naczyń krwionośnych siatkówki oka. W celu wyleczenia schorzenia używana jest laseroteriapa siatkówki. W niniejszej pracy analizowany przez nas będzie czas przeżycia w kontekście utraty ostrości wzroku oraz wpływu danych zmiennych na spowolnienie tego procesu. Zbiór danych składa się z następujących zmiennych:
...
W zbiorze danych nie występują żadne braki danych. 
...
Pierwsza dwa wykresy przedstawiają częstotliwość występowowania wartośći w zmiennych **age** oraz **time**. Zauważyć możemy, iż duża liczba pacjentów ze zdiagnozowaną chorbą znajduje się pomiędzy 8 a 20 rokiem życia. Najmniej zdiagnozowano osób w wieku powyżej 55 lat. Obok widoczny jest wykres częstotliowści liczby miesięcy do momentu utraty wzroku. Najczęstszymi wartościami czasu do utraty wzroku są 3 do 4 oraz 38 do 42 miesięcy od diagnozy. 

Kolejne wykresy przedstawiają rozkład dla zmiennych **eye**, **laser**, **status** oraz **trt**. W przypadku zmiennej eye mamy równą taką samą liczbę obserwacji dla prawego i lewego oka. Częstściej w leczeniu stosowany jest typ lasera xenonowego, niż argonowego. W zmiennej status przeważa liczba danych cenzurowanych (239), niż utraty wzroku (155). Ostatnia prezentowana na wykresie zmienna to trt. W tym przypadku mamy równą liczbę obserwacji z zastosowanie i nie zastosowaniem leczenia.
Ostatni analizowany powyżej wykres prezentuje rozkład częstości występowania danej grupy ryzyka (**risk**). Widać, że najczęściej przypisywana jest wartość 9 oraz 10. Najrzadszą wartością są grupy ryzyka na poziomie 6 oraz 8. 
...
Oferują nam one bardzo podobną interpretację jak wykresy prawdopodobieństw. Szansa na utratę wzroku bardzo szybko (znów po około 2 miesiącach) było większe od 0 i w dość równomierny sposób wzrastała do poziomu około 0.6, na którym to utrzymywała się od około 60 miesięcy do końca,czyli 75 miesięcy. Przejdziemy teraz do wyznaczenia krzywych dla predyktorów zawartych w naszym zbiorze danych - zmiennych `trt`, `age`, `laser`, `eye` oraz `risk`.
...
plot(mod.km.laser, xlab="Czas", ylab="Prawdopodobieństwo zachowania wzroku", col = c('black','grey'), main="Krzywa zachowania wzroku dla zmiennej laser")
lines(mod.na.laser, col=c('red','pink'))
legend(x = 'bottom', legend = c('Xenon Kaplan-Meier','Argon Kaplan-Meier','Xenon Nelson - Aalen','Argon Nelson - Aalen'), col = c('black','grey','red','pink'), pch=16)
...
legend(x = 'bottom', legend = c('Kaplan-Meier','Nelson-Aalen'), col = c('black','red'), pch=16)
plot(mod.km.risk, xlab="Czas", ylab="Prawdopodobieństwo zachowania wzroku", col = c('black','red','pink','grey','purple','green'), main="Krzywa zachowania wzroku dla zmiennej risk")
legend(x = 'bottom', legend = c('risk 6','risk 8','risk 9','risk 10','risk 11','risk 12'), col = c('black','red','pink','grey','purple','green'), pch=16)

```

**Krzywa zachowania wzroku zmiennej laser**
Na wykresie widać, że krzywe w przypadku obydwóch estymatorów są do siebie zbliżone. Również w przypadku obydwóch estymatorów po 60 miesiącu wzrasta prawdopodobieństwo zachowania wzroku w przypadku użycia lasera Xenonowego.

**Krzywa zachowania wzroku zmiennej eye**
Na wykresie widać, że krzywe dla oka są takie same niezależnie od użytego estymatora.

**Krzywa zachowania wzroku zmiennej risk**
Najmniejsze prawdopodobieństwo zachowania wzroku cechuje obserwacje z grupy ryzyka 10 i 12  w całym okresie badania. Największe prawdopodobieństwo zachowania wzorku wskazuje natomiast, właściwie, grupa 6 ryzyka.
...
Wyniki testów dla zmiennej kategoryzowanej Age wskazują, że nie występuje istotna różnica w przeżywalności jednostek w poszczególnych grupach wiekowych.
